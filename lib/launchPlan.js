 ...
 *
 * Adapters (all optional; default to the real process/fs/path):
 *   platform   process.platform — "win32" switches bin resolution + quoting
 *   env        process.env — OMPP_OMP_BIN, PATH, PATHEXT
 *   fsAdapter  fs — needs existsSync; may carry firstExisting(dir, names)
...
class LaunchPlan {
  ...
  /** First existing path among names, or null. Honors an adapter-provided
   *  firstExisting so tests can script discovery without a filesystem. */
  firstExisting(dir, names) {
    const custom = this.fsAdapter.firstExisting;
    if (typeof custom === "function") {
      return custom.call(this.fsAdapter, dir, names);
    }
