 ...
 * stdin is piped, and the launch / open / rename / delete / create action loop.
 ...
 *   store.reveal(dir)
 *     the filesystem work — and the file-manager reveal — belongs to the
 *     store module.
 ...
 * a TTY, nor the siblings to exist yet. The sibling module-shape sniffing
 * (class ?? factory ?? namespace ?? default) is gone: both are first-party
 * modules with stable named exports, so the defaults destructure the
 * contract instead of probing it.
 */
...
// The sibling modules are resolved lazily (never at require time); each
// exports a named class this repo owns, so the contract is direct.
...
function defaultStore(registry) {
  const { ModeStore } = require("./store");

  return new ModeStore({ registry });
}
...
class Picker {
  ...
  async _pickInteractive(modes) {
    ...
    for (;;) {
      ...
      // Reveal is fire-and-forget in the store: the loop drops back to
      // the mode list, exactly like rename and delete.
      if (action === "open") {
        this.store.reveal(chosen.dir);
        this._log(`opened ${chosen.dir}`);
        continue;
      }
